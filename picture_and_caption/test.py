...
def main(_):
    with tf.Graph().as_default():
        ...
        vgg_output, end_points = vgg_16(images, is_training=False)
        ...
        with slim.arg_scope(
            ...
            image_embeddings = slim.fully_connected(inputs=vgg_output,
                                                    num_outputs=256,
                                                    activation_fn=None,
                                                    scope='image_embeddings')
        ...
        with tf.variable_scope(
            ...
            zero_state = lstm_cell.zero_state(batch_size=1,
                                              dtype=tf.float32)

            y, z = lstm_cell(image_embeddings, zero_state)

            tf.concat(values=z, axis=1, name='test')
